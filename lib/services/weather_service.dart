...
class WeatherService {
  static Future<WeatherData> fetchWeather({
    ...
    final responses = await Future.wait([
      http.get(stdUrl),
      http.get(ecmwfUrl).catchError((_) => http.Response('{}', 404)),
      http.get(aqiUrl).catchError((_) => http.Response('{}', 404)),
    ]);
    ...
    if (stdRes.statusCode != 200) {
      throw Exception('मौसम डेटा प्राप्त करने में त्रुटि: ${stdRes.statusCode}');
    }
    ...
    final double maxPrecip = mathMax(precip1, precip2);
    ...
    for (int i = 0; i < stdTimes.length; i++) {
      ...
      // Calculate realistic precipitation probability based on actual rain volume & cloud cover
